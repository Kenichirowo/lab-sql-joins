...
SELECT c.name AS category_name, COUNT(fc.film_id) AS number_of_films FROM sakila.category AS c
...
SELECT c.name AS category_name, AVG(f.length) AS average_running_time FROM sakila.film AS f
JOIN sakila.film_category AS fc 
...
SELECT COUNT(*) > 0 AS is_available FROM sakila.inventory AS i
JOIN sakila.film AS f 
ON i.film_id = f.film_id
